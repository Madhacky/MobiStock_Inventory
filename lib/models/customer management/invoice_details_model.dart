...
class InvoiceDues {
  ...
  factory InvoiceDues.fromJson(Map<String, dynamic> json) {
    return InvoiceDues(
      ...
      partialPayments: (json['partialPayments'] as List<dynamic>?)
              ...
              .toList() ??
          [],
      creationDate: (json['creationDate'] as List<dynamic>?)?.cast<int>() ?? [],
      paymentRetriableDate: (json['paymentRetriableDate'] as List<dynamic>?)?.cast<int>() ?? [],
  ...
  String get formattedCreationDate {
    if (creationDate.length >= 3) {
      return "${creationDate[2]} ${_getMonthName(creationDate[1])} ${creationDate[0]}";
    }
    return '';
  }
...
return partialPayments;
    // return partialPayments.where((payment) => payment.paidAmount > 0).toList();
  }
...
class PartialPayment {
  ...
  factory PartialPayment.fromJson(Map<String, dynamic> json) {
    return PartialPayment(
      ...
      paidDate: (json['paidDate'] as List<dynamic>?)?.cast<int>() ?? [],
  ...
  String get formattedPaidDate {
    if (paidDate.length >= 3) {
      return "${paidDate[2]} ${_getMonthName(paidDate[1])} ${paidDate[0]}";
    }
    return '';
  }
...
class InvoiceEmi {
  // Add EMI fields if needed based on your requirements
  final int? id;
