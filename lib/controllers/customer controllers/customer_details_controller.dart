      ...
      if (response != null && response.statusCode == 200) {
        ...
        if (notifyResponse.status == 'Success') {
          if (notifyResponse.payload.notified.contains(customerId)) {
            ...
            Get.snackbar(
              'Warning',
              'Customer not found for notification',
            ...
            Get.snackbar(
              'Info',
              'Notification processed but status unclear',
  ...
  Future<void> processPartialPayment(
    ...
    try {
      ...
      if (response != null && response.statusCode == 200) {
        ...
        Get.snackbar(
          'Success',
          'Payment of ₹${amount.toStringAsFixed(0)} processed successfully!',
          backgroundColor: Colors.green,
          colorText: Colors.white,
          icon: Icon(Icons.check_circle, color: Colors.white),
          duration: Duration(seconds: 3),
      ...
      rethrow; // Rethrow to let the dialog handle the error state
    } finally {
