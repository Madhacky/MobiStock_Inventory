...
class GenerateInventoryController extends GetxController {
  ...
  // API endpoint
  static const String generateInventoryUrl =
      'https://backend-production-91e4.up.railway.app/generate';
  ...
  Future<void> generateInventoryLink() async {
    try {
      ...
      if (response != null && response.statusCode == 200) {
        ...
        throw Exception(
          'Failed to generate link. Status: ${response?.statusCode}',
        );
      }
    } catch (error) {
      hasError.value = true;
  ...
  Future<void> shareInventoryLink() async {
    ...
    try {
      await Share.share(generatedLink.value, subject: 'My Inventory Link');
